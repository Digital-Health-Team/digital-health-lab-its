<div class="max-w-5xl mx-auto space-y-8 pb-10">
    ...
    {{-- LOGIC: GET RELATED PROJECT CONTEXT --}}
    @php
        // Mencari konteks project dari report pertama yang ada di absensi ini
    ...
    {{-- HEADER NAVIGATION (CHILD OF PROJECT) --}}
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <div class="text-xs breadcrumbs text-gray-500 mb-1">
                <ul>
                    ...
                    @if ($relatedProject)
                        <li><a href="{{ $backRoute }}"
                                class="font-semibold text-primary hover:underline">{{ $pName }}</a></li>
                    @endif
                    <li class="font-bold">Work Log</li>
                </ul>
            </div>
            ...
            <a href="{{ $backRoute }}"
                ...
                Back to {{ $relatedProject ? 'Project' : 'List' }}
            </a>

            <h1 class="text-2xl md:text-3xl font-black mt-1 dark:text-gray-100">Attendance & Work Log</h1>

            <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-2">
                <x-avatar :image="$attendance->user->profile_photo
                    ? asset('storage/' . $attendance->user->profile_photo)
                    : null" class="w-6 h-6 mr-1 shadow-sm" />
                <span class="font-bold text-gray-800 dark:text-gray-200">{{ $attendance->user->name }}</span>
        ...
        <div
            class="flex items-center gap-4 bg-base-200/50 dark:bg-gray-800 p-4 rounded-xl border border-base-300 dark:border-gray-700">
            ...
            <div
                class="badge p-4 font-bold shadow-sm {{ $attendance->check_out ? 'badge-success text-white' : 'badge-warning text-black' }}">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            {{-- Tombol Back mengarah ke Index Admin --}}
            <a href="{{ route('admin.attendance') }}"
                class="btn btn-ghost btn-sm pl-0 gap-2 text-gray-500 hover:bg-transparent">
                <x-icon name="o-arrow-left" class="w-4 h-4" /> Back to Attendances
            </a>

            <h1 class="text-2xl md:text-3xl font-black mt-2 dark:text-gray-100">Attendance Report</h1>

            <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                <x-avatar :image="$attendance->user->profile_photo
                    ? asset('storage/' . $attendance->user->profile_photo)
                    : null" class="w-6 h-6 mr-1" />
                <span class="font-bold text-gray-800 dark:text-gray-200">{{ $attendance->user->name }}</span>
                <span>•</span>
                <x-icon name="o-calendar" class="w-4 h-4" />
                {{ $attendance->created_at->translatedFormat('l, d F Y') }}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {{-- LEFT COLUMN: TIME & LOCATION & LOGS --}}
                                    ...
                                    <div
                                        ...
                                        <div class="flex justify-between items-start mb-2 gap-2">
                                            <div class="flex-1">
                                                ...
                                                <a href="{{ route('admin.jobdesks.revision', $report->jobdesk_id) }}"
                                                ...
                                                <div class="flex flex-wrap gap-3">
                                                    @foreach ($report->attachments as $att)
                                                        ...
                                                        <div @click.prevent="$dispatch('open-lightbox', { url: '{{ $url }}', index: {{ $globalIndex !== false ? $globalIndex : 0 }} })"
                                                            class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-gray-800 border border-base-200 dark:border-gray-700 text-xs hover:border-primary hover:text-primary transition cursor-pointer group/att max-w-[200px]">
                                                            <div
                                                            ...
                                                            <div class="flex flex-col min-w-0">
                                                                <span
                                                                    class="font-bold truncate">{{ $att->file_name }}</span>
                                                                <span
