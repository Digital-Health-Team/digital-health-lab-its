        ...
        <div class="w-80 xl:w-96 shrink-0 sticky top-20">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div class="border-t border-slate-100 dark:border-slate-800 px-5 py-4 space-y-3">
                    ...
                    {{-- Unique Code + QR --}}
                    @if($activeTool->unique_code)
                    @php
                        $_scanUrl = route('scan.tool', $activeTool->unique_code);
                    ...
                    @endphp
                    <div x-data="{ qrOpen: false }" class="flex items-start gap-3">
                        {{-- QR thumbnail — click to full preview --}}
                        <button type="button" @click="qrOpen = true"
                            title="{{ __('Klik untuk preview QR') }}"
                            class="w-16 h-16 shrink-0 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-white p-1 cursor-zoom-in hover:ring-2 hover:ring-indigo-400 transition-all">
                            {!! $_qrSvg !!}
                        </button>
                        <div class="flex-1 min-w-0">
                            <p class="text-[10px] uppercase tracking-wider font-bold text-slate-400 dark:text-slate-500">{{ __('Kode Unik') }}</p>
                            <p class="text-xs font-mono font-bold text-slate-700 dark:text-slate-300 break-all leading-snug mt-0.5">{{ $activeTool->unique_code }}</p>
                            <a href="{{ $_scanUrl }}" target="_blank"
                                class="inline-flex items-center gap-1 mt-1.5 text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">
                        ...
                        <div x-show="qrOpen" x-cloak
                            ...
                            <div @click.stop
                                ...
                                <div class="text-center space-y-0.5">
                                    <p class="text-[10px] uppercase tracking-widest font-bold text-slate-400">{{ __('Kode Unik') }}</p>
                                    <p class="font-mono text-sm font-bold text-slate-700 dark:text-slate-200 break-all">{{ $activeTool->unique_code }}</p>
                                </div>
                                <a href="{{ $_scanUrl }}" target="_blank"
                    ...
                    {{-- Print Label --}}
                    @if($activeTool->unique_code)
                    <a href="{{ route('admin.print-label', ['type' => 'tool', 'id' => $activeTool->id]) }}"
                        target="_blank"
                        class="mt-1 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold
                               bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-500/10
                               text-slate-600 dark:text-slate-400 hover:text-indigo-700 dark:hover:text-indigo-300
                               border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500/40
                               transition-colors">
