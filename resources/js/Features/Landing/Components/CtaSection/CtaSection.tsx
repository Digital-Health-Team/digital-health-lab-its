 ...
 * near-midnight world (bg-primary-950 → Contact's rgba(3,16,38,…) gradient)
 * but owns the typographic statement + ECG heartbeat horizon; Contact owns
 * the form.
 */
export default function CtaSection() {
    ...
    return (
        <Box
            ...
            id="explore"
            ref={containerRef}
            className="relative bg-primary-950 py-[clamp(96px,16vh,160px)] px-6 md:px-12 overflow-hidden"
        >
            ...
            <Box className="relative z-10 max-w-4xl mx-auto flex flex-col items-center text-center">
                ...
                <Box className="cta-actions flex flex-col sm:flex-row items-center gap-6 sm:gap-9">
                    ...
                    <a
                        href={products().url}
                        className="group font-body font-semibold text-base text-secondary-400 underline-offset-4 hover:underline transition-colors duration-300 hover:text-secondary-300 focus-visible:outline-2 focus-visible:outline-secondary-400 focus-visible:outline-offset-4 rounded-sm flex items-center gap-2"
                    >
                        {secondaryLabel}
                        <Text
                            as="span"
