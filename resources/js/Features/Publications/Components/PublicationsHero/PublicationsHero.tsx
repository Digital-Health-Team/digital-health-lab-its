...
interface PublicationsHeroProps {
    data: PublicationsHero;
}

export default function PublicationsHero({ data }: PublicationsHeroProps) {
    const smallImage = data.images[0];
    const mainImage = data.images[1];
    ...
    return (
        ...
        >
            ...
            <Box className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 h-full">
                ...
                {/* LEFT — eyebrow + small image + heading + body */}
                        ...
                        <Box className="mb-5 h-32 sm:h-40 md:h-48 w-[65%] rounded-2xl overflow-hidden shadow-card-soft">
                            {smallImage?.src ? (
                                <Box
                                    ...
                                    style={{
                                        backgroundImage: `url(${smallImage.src})`,
